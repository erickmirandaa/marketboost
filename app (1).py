...
def atualizar_grafico(palavra_chave_selecionada):
    ...
    # Escolher as métricas a serem exibidas no gráfico
    metricas_grafico = {k: v for k, v in metricas.items() if k not in ['ROAS', 'ROAS Direto', 'ACOS', 'ACOS Direto']}
    
    metricas_labels = list(metricas_grafico.keys())
    metricas_values = list(metricas_grafico.values())
